 ...
 * AdminDashboard.jsx
 * Corrections :
 *  1. Gestion des années académiques accessible depuis le header (pas seulement dans le modal classe)
 *  2. Création d'une nouvelle année libre à tout moment via un bouton "Gérer les années"
 *  3. Upload de thumbnail PENDANT la création du cours (pas après)
 *     → le cours est créé en 2 étapes : d'abord info, puis thumbnail, puis fermer
 */
...
/* ── ThumbnailUploader inline (pas besoin d'importer le composant externe) ── */
function ThumbnailField({ courseId, current, onUploaded }) {
  ...
  const upload = async (file) => {
    ...
    if (!file.type.startsWith('image/')) {
      alert('Seulement les images sont acceptées (JPG, PNG, WEBP)')
      return
    }
    ...
    try {
      ...
      if (courseId) {
        ...
        onUploaded?.(data.thumbnail || data.url)
      } else {
        // Pas encore d'ID → stocker le fichier pour l'envoyer après création
        onUploaded?.(file)
      }
    } catch (err) {
      ...
      >
        {preview ? (
          <>
            ...
            >
              <span style={{ color: '#fff', fontWeight: 700, fontSize: 13 }}>🔄 Changer l'image</span>
            </div>
          </>
        ) : uploading ? (
          ...
          <div style={{ textAlign: 'center', padding: '12px 20px' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }}>
              Cette image s'affichera en grande icône sur la page des cours
            </div>
          </div>
        )}
        <input
...
export default function AdminDashboard() {
  ...
  const [yearModal,   setYearModal]   = useState(false) // ← NOUVEAU : modal années indépendant
  ...
  const [editYear,    setEditYear]    = useState(null) // ← édition d'une année existante
  ...
  const [pendingThumb,   setPendingThumb]   = useState(null) // fichier image en attente si pas encore d'ID
  ...
  /* ══════════════════════════════════════════════
     ANNÉES ACADÉMIQUES — gestion complète
  ══════════════════════════════════════════════ */
  const openYearModal = () => {
    setEditYear(null)
  ...
  const saveYear = async () => {
    ...
    try {
      let data
      if (editYear) {
        const r = await api.put(`/academic/years/${editYear.id}`, {
        ...
        data = r.data
        flash(`Année ${data.name} modifiée !`)
      } else {
        const r = await api.post('/academic/years', {
          name:       newYearForm.name,
          start_date: new Date(newYearForm.start_date).toISOString(),
          end_date:   new Date(newYearForm.end_date).toISOString(),
          is_current: newYearForm.is_current,
      ...
      const updated = editYear
        ? years.map(y => y.id === editYear.id ? data : (newYearForm.is_current ? { ...y, is_current: false } : y))
        : [...years, data].sort((a, b) => new Date(b.start_date) - new Date(a.start_date))
      setYears(updated)
      setEditYear(null)
      setNewYearForm({ name: '', start_date: '', end_date: '', is_current: false })
    } catch (err) {
  ...
  const deleteYear = async (yr) => {
    ...
    try {
      ...
      setYears(years.filter(y => y.id !== yr.id))
  ...
  const setCurrentYear = async (yr) => {
    try {
      await api.patch(`/academic/years/${yr.id}/set-current`)
      setYears(years.map(y => ({ ...y, is_current: y.id === yr.id })))
      flash(`${yr.name} définie comme année courante`)
    } catch (err) {
      // Fallback si l'endpoint n'existe pas : mettre à jour localement
      setYears(years.map(y => ({ ...y, is_current: y.id === yr.id })))
      flash(`${yr.name} définie comme courante (local)`)
    }
  }

  /* Inline dans le modal classe */
  ...
  const saveCourse = async e => {
    ...
    try {
      if (editCourse) {
        ...
        // Si une image était déjà choisie avant la création → l'uploader maintenant
        if (pendingThumb instanceof File) {
          try {
            const fd = new FormData()
            fd.append('file', pendingThumb)
            const { data: td } = await api.post(
              `/admin/courses/${newCourse.id}/thumbnail`,
              fd,
              { headers: { 'Content-Type': 'multipart/form-data' } }
            )
            setEditCourse(c => ({ ...c, thumbnail: td.thumbnail || td.url }))
            setPendingThumb(null)
            flash('Cours créé avec image !', 'success')
          } catch { /* l'image sera uploadable manuellement */ }
        }
      }
    } catch (err) { flash(err.response?.data?.detail || 'Erreur', 'error') }
  }
  ...
  return (
    <>
        ...
        <>
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {/* ← NOUVEAU : bouton dédié gestion des années */}
              <button
          ...
          <div className="modal" style={{ maxWidth: 600, width: '95vw', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div className="modal-body">
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                {years.length === 0 ? (
                  <div style={{ color: 'var(--text-muted)', fontSize: 13, padding: '16px', background: '#f8fafc', borderRadius: 10 }}>
                    Aucune année académique. Créez-en une ci-dessous.
                  </div>
                ) : years.map(yr => (
                  <div key={yr.id} style={{
                    display: 'flex', alignItems: 'center', gap: 12,
                    padding: '12px 14px', borderRadius: 10, marginBottom: 8,
                    border: `1.5px solid ${yr.is_current ? '#22c55e' : 'var(--border)'}`,
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--navy)' }}>
                        {yr.name}
                        {yr.is_current && (
                          <span style={{ marginLeft: 8, fontSize: 10, background: '#22c55e', color: '#fff', padding: '2px 8px', borderRadius: 20, fontWeight: 800 }}>
                            COURANTE
                          </span>
                        )}
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setNewYearForm({
                            name:       yr.name,
                            start_date: yr.start_date ? yr.start_date.slice(0, 10) : '',
                            end_date:   yr.end_date   ? yr.end_date.slice(0, 10)   : '',
              ...
              <div style={{ padding: 16, background: '#f0f9ff', borderRadius: 12, border: '1px solid #bae6fd' }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--navy)', marginBottom: 14 }}>
                  {editYear ? `✏️ Modifier "${editYear.name}"` : '➕ Créer une nouvelle année'}
                </div>
                <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" className="btn btn-primary"
                    onClick={saveYear} disabled={savingYear} style={{ flex: 1 }}>
                    {savingYear ? 'Enregistrement...' : editYear ? '✓ Enregistrer les modifications' : '✓ Créer l\'année'}
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* ════════════ MODAL COURS (avec upload image intégré) ════════════ */}
      {courseModal && (
        <div className="modal-overlay" onClick={() => setCourseModal(false)}>
          <div className="modal" style={{ maxWidth: 580, width: '95vw', maxHeight: '92vh', overflowY: 'auto' }}
            onClick={e => e.stopPropagation()}>
            <div className="modal-header">
            ...
            <form onSubmit={saveCourse}>
              <div className="modal-body">
                ...
                {/* ── Image du cours EN HAUT ── */}
                <ThumbnailField
                  courseId={savedCourseId}
                  current={editCourse?.thumbnail}
                  onUploaded={val => {
                    if (val instanceof File) {
                      // Pas encore d'ID → stocker le fichier
                      setPendingThumb(val)
                      // Preview local
                      setEditCourse(c => c ? c : {})
                    } else {
                      // URL retournée par le backend
                      setEditCourse(c => ({ ...(c || {}), thumbnail: val }))
                    }
                  }}
                />
                ...
                {savedCourseId && (
                  <div style={{ padding: '10px 14px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#16a34a', fontSize: 13, fontWeight: 600 }}>
                    ✓ Cours créé ! Vous pouvez modifier l'image puis fermer.
                  </div>
                )}
              </div>
              <div className="modal-footer">
...
/* ══════════════════════════════════════════════════════════
   SOUS-COMPOSANTS (ClassCard, ClassDetailView, UsersView, ResultsTab)
   Identiques à l'original — aucune modification nécessaire
══════════════════════════════════════════════════════════ */
