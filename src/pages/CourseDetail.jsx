...
export default function CourseDetail() {
  ...
  // ── FIX #3 : flash avec timer annulable ──
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setCourse(null)
        setError({ status: err.response?.status, detail: err.response?.data?.detail || err.message || 'Erreur inconnue' })
      } finally {
        setLoading(false)
      }
    }
  ...
  }, [id, homeworks.length, exams.length, sessions.length])

  const switchTab = useCallback(t => { setTab(t); loadTab(t) }, [loadTab])

  // ── Leçons ──
  const reloadLessons = useCallback(async () => {
    const r = await api.get(`/courses/${id}/lessons`).catch(() => ({ data: [] }))
    setLessons(r.data)
  }, [id])
  ...
  // ── FIX #7 : try/catch sur deleteLesson ──
  ...
  if (!course) return (
    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
      ...
      <p style={{ color: '#94a3b8', fontSize: 12, marginBottom: 24 }}>ID du cours : {id}</p>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
        <button className="btn btn-outline" onClick={() => navigate(-1)}>← Retour</button>
        <button className="btn btn-primary" onClick={() => navigate('/courses')}>Catalogue des cours</button>
      </div>
    </div>
  )
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', padding: '24px 28px', display: 'flex', flexDirection: 'column', minHeight: 200, justifyContent: 'space-between' }}>
          <button onClick={() => navigate(-1)} style={{ alignSelf: 'flex-start', background: 'rgba(255,255,255,.18)', border: 'none', color: '#fff', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, backdropFilter: 'blur(6px)' }}>
            ← Retour
          </button>
          <div style={{ marginTop: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,.8)' }}>👨‍🏫 {course.teacher_name || 'Non assigné'}</span>
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,.8)' }}>📖 {lessons.length} leçon{lessons.length > 1 ? 's' : ''}</span>
              {course.student_count > 0 && <span style={{ fontSize: 13, color: 'rgba(255,255,255,.8)' }}>👥 {course.student_count}</span>}
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 24, overflowX: 'auto' }}>
        {TABS.map(({ key, label }) => (
          <button key={key} onClick={() => switchTab(key)} style={{ padding: '10px 18px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 500, whiteSpace: 'nowrap', borderBottom: tab === key ? '2px solid var(--blue)' : '2px solid transparent', color: tab === key ? 'var(--blue)' : 'var(--text-muted)' }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div>
          {canManage && (
            <div style={{ marginBottom: 16 }}>
              <button className="btn btn-primary btn-sm" onClick={() => { setLessonForm({ title: '', duration: '', order: lessons.length }); setLessonFile(null); setUploadModal(true) }}>
                + Ajouter une leçon
              </button>
            </div>
          )}
          {lessons.length === 0 ? (
            <div className="empty-state"><h3>Aucune leçon</h3><p>{canManage ? 'Ajoutez la première leçon.' : "L'enseignant n'a pas encore ajouté de leçons."}</p></div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {lessons.map((l, i) => (
                <div key={l.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid var(--border)', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14, transition: 'box-shadow .15s' }}
                  onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 16px rgba(59,130,246,.1)'}
                  onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
                >
                  <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: l.type === 'pdf' ? '#dbeafe' : '#dcfce7', color: l.type === 'pdf' ? '#1d4ed8' : '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
                    {l.type === 'pdf' ? '📄' : '🎬'}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--navy)', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', background: '#f1f5f9', padding: '1px 7px', borderRadius: 6, flexShrink: 0 }}>#{i + 1}</span>
                      {l.title}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }}>{l.type === 'pdf' ? 'PDF' : 'Vidéo'}{l.duration ? ` · ${l.duration}` : ''}</div>
                  </div>
                  <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 600, flexShrink: 0, background: l.file_path ? '#dcfce7' : '#fee2e2', color: l.file_path ? '#16a34a' : '#991b1b' }}>
                    {l.file_path ? '✓ Disponible' : '⚠ Manquant'}
                  </span>
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    {l.file_path && <button className="btn btn-primary btn-sm" onClick={() => navigate(`/lesson/${l.id}`)}>{isStudent ? 'Ouvrir' : 'Aperçu'}</button>}
      ...
      {tab === 'homeworks' && (
        <div>
          {canManage && <div style={{ marginBottom: 16 }}><button className="btn btn-primary btn-sm" onClick={() => navigate('/homeworks')}>Gérer les devoirs →</button></div>}
          {homeworks.length === 0 ? (
              ...
              <div key={hw.id} className="card" style={{ marginBottom: 10 }}>
                <div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {hw.has_file && <a href={`/api/homeworks/${hw.id}/file`} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: 'var(--blue)', textDecoration: 'none', display: 'block', marginTop: 4 }}>📎 Fichier joint</a>}
      ...
      {tab === 'exams' && (
        <div>
          {canManage && <div style={{ marginBottom: 16 }}><button className="btn btn-primary btn-sm" onClick={() => navigate('/exams')}>Gérer les examens →</button></div>}
          {exams.length === 0 ? (
      ...
      {tab === 'sessions' && (
        <div>
          {canManage && (
            <div style={{ marginBottom: 16 }}>
              <button className="btn btn-primary btn-sm" onClick={() => setSessionModal(true)}>+ Planifier un cours en ligne</button>
            </div>
          )}
          {sessions.length === 0 ? (
            ...
            <div key={s.id} className="card" style={{ marginBottom: 10 }}>
              <div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {s.scheduled_at && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>📅 {new Date(s.scheduled_at).toLocaleString('fr-FR')}</div>}
                </div>
            ...
            <form onSubmit={uploadLesson}>
              <div className="modal-body">
                <div className="form-group">
                  ...
                  <input className="form-input" required value={lessonForm.title} onChange={e => setLessonForm({ ...lessonForm, title: e.target.value })} placeholder="Ex : Introduction à la cinématique" />
                </div>
                ...
                <div className="form-group">
                  ...
                  >
                    {lessonFile ? (
                      <><div style={{ fontSize: 28, marginBottom: 6 }}>📎</div><div style={{ fontWeight: 600 }}>{lessonFile.name}</div><p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{(lessonFile.size / 1024 / 1024).toFixed(1)} MB</p></>
                    ) : (
                      <><div style={{ fontWeight: 600, marginTop: 8 }}>Glisser-déposer ou cliquer</div><p style={{ fontSize: 12, color: 'var(--text-muted)' }}>PDF, MP4, WebM acceptés</p></>
                    )}
