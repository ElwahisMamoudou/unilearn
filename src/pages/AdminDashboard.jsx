...
export default function AdminDashboard() {
  ...
  const deleteYear = async yr => {
    ...
    } catch (err) { flash(err.response?.data?.detail || 'Erreur suppression','error') }
  }
  ...
  const openCreateCourse = () => {
    ...
    setCourseForm({ title:'', description:'', category_id:'', teacher_id: classDetail?.teacher_id ? String(classDetail.teacher_id) : '', is_published:true })
    setCourseModal(true)
  }
  ...
  const saveCourse = async e => {
    ...
    try {
      if (editCourse) {
        ...
        flash('Cours modifie !'); setCourseModal(false); openClass(selClass)
      } else {
        ...
        if (pendingThumb instanceof File) {
          try {
            ...
            const { data: td } = await api.post(`/admin/courses/${newCourse.id}/thumbnail`, fd, { headers: { 'Content-Type':'multipart/form-data' } })
  ...
  const deleteCourse = async id => {
    ...
    try { await api.delete(`/admin/courses/${id}`); flash('Cours supprime'); openClass(selClass) }
    catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  ...
  const togglePublish = async c => {
    ...
    flash(c.is_published ? 'Cours depublie' : 'Cours publie'); openClass(selClass)
  }

  const loadEnrolled = useCallback(async courseId => {
    const r = await api.get(`/admin/courses/${courseId}/students`)
    setEnrolled(r.data); setEnrollCourseId(courseId)
  }, [])

  const enrollStudent = async studentId => {
    try { await api.post(`/admin/courses/${enrollCourseId}/enroll`, { student_ids:[studentId] }); flash('Etudiant inscrit'); loadEnrolled(enrollCourseId) }
    catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  const unenrollStudent = async studentId => {
    await api.delete(`/admin/courses/${enrollCourseId}/students/${studentId}`)
    flash('Etudiant retire'); loadEnrolled(enrollCourseId)
  }
  ...
  const confirmAddStudents = async () => {
    ...
    try {
      ...
      flash(`${data.added} etudiant(s) ajoute(s)`); setStudModal(false); openClass(selClass)
    } catch { flash('Erreur','error') }
  }
  const removeStudentFromClass = async (sid, name) => {
    if (!confirm(`Retirer ${name} de la classe ?`)) return
    try { await api.delete(`/classes/${classDetail.id}/students/${sid}`); flash(`${name} retire(e)`); openClass(selClass) }
    catch { flash('Erreur','error') }
  }
    ...
    <>
          ...
          {filteredClasses.length===0 ? (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:16 }}>
              {filteredClasses.map(cls => (
                <ClassCard key={cls.id} cls={cls} onClick={() => openClass(cls)} onEdit={e => openEditClass(cls,e)} onDelete={e => deleteClass(cls.id,e)}/>
              ))}
            </div>
          )}
        </>
      ...
      {view==='classDetail' && selClass && (
        <ClassDetailView
          ...
          enrolled={enrolled} enrollCourseId={enrollCourseId} availableForEnroll={availableForEnroll}
            ...
            <div className="modal-body">
                  ...
                  <div key={yr.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:10, marginBottom:8, border:`1.5px solid ${yr.is_current?'#22c55e':'var(--border)'}`, background: yr.is_current?'#f0fdf4':'white' }}>
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:2 }}>
                        {yr.start_date ? new Date(yr.start_date).toLocaleDateString('fr-FR') : '?'} → {yr.end_date ? new Date(yr.end_date).toLocaleDateString('fr-FR') : '?'}
                      </div>
                    </div>
                    <div style={{ display:'flex', gap:6 }}>
                      {!yr.is_current && (
                        <button className="btn btn-outline btn-sm" style={{ fontSize:11 }} onClick={() => setCurrentYear(yr)}>Definir courante</button>
                      )}
              ...
              <div style={{ padding:16, background:'#f0f9ff', borderRadius:12, border:'1px solid #bae6fd' }}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" value={newYearForm.name} onChange={e=>setNewYearForm(f=>({...f,name:e.target.value}))} placeholder="Ex : 2025-2026"/>
                </div>
                <div className="form-row">
                  <div className="form-group">
                    <label className="form-label">Date de debut *</label>
                    <input className="form-input" type="date" value={newYearForm.start_date} onChange={e=>setNewYearForm(f=>({...f,start_date:e.target.value}))}/>
                  </div>
                  <div className="form-group">
                    <label className="form-label">Date de fin *</label>
                    <input className="form-input" type="date" value={newYearForm.end_date} onChange={e=>setNewYearForm(f=>({...f,end_date:e.target.value}))}/>
                  </div>
                </div>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  {editYear && (
                    <button className="btn btn-outline btn-sm" onClick={() => { setEditYear(null); setNewYearForm({name:'',start_date:'',end_date:'',is_current:false}) }}>Annuler</button>
                  )}
        ...
        <div className="modal-overlay" onClick={() => setClassModal(false)}>
          <div className="modal" style={{ maxWidth:560, width:'95vw', maxHeight:'92vh', overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group">
                <label className="form-label">Nom *</label>
                <input className="form-input" value={classForm.name} onChange={e=>setClassForm(f=>({...f,name:e.target.value}))} placeholder="Ex : L1 Informatique"/>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Code</label>
                  <input className="form-input" value={classForm.code} onChange={e=>setClassForm(f=>({...f,code:e.target.value}))} placeholder="Ex : L1-INFO"/>
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <textarea className="form-input" rows={2} value={classForm.description} onChange={e=>setClassForm(f=>({...f,description:e.target.value}))} style={{ resize:'vertical' }}/>
              </div>
              <div className="form-group">
                ...
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  ...
                  <button type="button" className="btn btn-outline btn-sm" style={{ whiteSpace:'nowrap', flexShrink:0 }} onClick={() => setShowNewYear(v=>!v)}>
                ...
                {showNewYear && (
                  <div style={{ marginTop:12, padding:16, background:'#f0f9ff', borderRadius:10, border:'1px solid #bae6fd' }}>
                    ...
                    <button type="button" className="btn btn-primary btn-sm" onClick={createYearInline} disabled={savingYear} style={{ width:'100%' }}>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="form-input" type="number" min={1} value={classForm.max_students} onChange={e=>setClassForm(f=>({...f,max_students:parseInt(e.target.value)}))}/>
                </div>
                ...
                {!editUser && (
                  <>
                    ...
                    <div className="form-group">
                      ...
                      <input className="form-input" type="password" required value={userForm.password} onChange={e=>setUserForm({...userForm,password:e.target.value})}/>
                    </div>
                  </>
                )}
                <div className="form-group">
            ...
            <form onSubmit={saveCourse}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" required value={courseForm.title} onChange={e=>setCourseForm({...courseForm,title:e.target.value})} placeholder="Ex : Introduction aux reseaux"/>
                </div>
                <div className="form-group">
                  <label className="form-label">Description</label>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <select className="form-select" required value={courseForm.teacher_id} onChange={e=>setCourseForm({...courseForm,teacher_id:e.target.value})}>
                      <option value="">-- Choisir --</option>
                      {teachers.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
                    ...
                    <select className="form-select" value={courseForm.category_id} onChange={e=>setCourseForm({...courseForm,category_id:e.target.value})}>
                ...
                <div className="form-group">
                  ...
                  <select className="form-select" value={courseForm.is_published} onChange={e=>setCourseForm({...courseForm,is_published:e.target.value==='true'})}>
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-outline" onClick={closeCourseModal}>{savedCourseId?'Fermer':'Annuler'}</button>
                {!savedCourseId ? (
      ...
      {studModal?.open && (
        <div className="modal-overlay" onClick={() => setStudModal(false)}>
          <div className="modal" style={{ maxWidth:520, width:'95vw', maxHeight:'85vh', overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">Ajouter des etudiants</span>
              <button className="modal-close" onClick={() => setStudModal(false)}>×</button>
            </div>
            <div className="modal-body">
              <input className="form-input" placeholder="Rechercher..." value={studSearch} onChange={e=>setStudSearch(e.target.value)} style={{ marginBottom:10 }}/>
              <div style={{ maxHeight:300, overflowY:'auto', border:'1px solid var(--border)', borderRadius:8 }}>
                {(studModal.available||[])
                  ...
                  .map(s => (
                    <label key={s.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderBottom:'1px solid #f1f5f9', cursor:'pointer', background:selectedStuds.includes(s.id)?'#eff6ff':'white' }}>
                      <input type="checkbox" checked={selectedStuds.includes(s.id)} onChange={e=>setSelectedStuds(p=>e.target.checked?[...p,s.id]:p.filter(x=>x!==s.id))}/>
                      <div>
              ...
              {selectedStuds.length>0 && <div style={{ marginTop:8, fontSize:13, color:'#3b82f6', fontWeight:600 }}>{selectedStuds.length} selectionne(s)</div>}
            </div>
            <div className="modal-footer">
              <button className="btn btn-outline" onClick={() => setStudModal(false)}>Annuler</button>
              <button className="btn btn-primary" onClick={confirmAddStudents} disabled={!selectedStuds.length}>Ajouter ({selectedStuds.length})</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:12 }}>
          <div>
            ...
            <div style={{ fontSize:13, opacity:.75, display:'flex', gap:16, flexWrap:'wrap' }}>
              ...
              {(classDetail?.academic_year_name||selClass.academic_year_name) && <span>📅 {classDetail?.academic_year_name||selClass.academic_year_name}</span>}
            </div>
          </div>
      ...
      <div style={{ display:'flex', gap:0, marginBottom:24, borderBottom:'1px solid var(--border)', overflowX:'auto' }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setDetailTab(t.id)} style={{ padding:'10px 18px', border:'none', cursor:'pointer', fontSize:13, whiteSpace:'nowrap', fontWeight:detailTab===t.id?700:400, color:detailTab===t.id?'var(--navy)':'var(--text-muted)', background:'transparent', borderBottom:detailTab===t.id?'2px solid var(--navy)':'2px solid transparent', marginBottom:-1 }}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          {detailTab==='courses' && (
            <div>
              ...
              {(classDetail.courses?.length??0)===0 ? (
                <div className="empty-state"><h3>Aucun cours</h3></div>
              ) : (
                    ...
                    <div key={course.id} className="card">
                      <div className="card-body" style={{ display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' }}>
                        {thumbUrl(course.thumbnail) ? (
                          <img src={thumbUrl(course.thumbnail)} alt={course.title} style={{ width:56, height:56, borderRadius:10, objectFit:'cover', flexShrink:0 }}/>
                        ) : (
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>{course.teacher_name} · {course.lesson_count??0} lecon(s) · {course.student_count??0} etudiant(s)</div>
                        </div>
          ...
          {detailTab==='students' && (
            <div>
              ...
              {(classDetail.students?.length??0)===0 ? (
                <div className="empty-state"><h3>Aucun etudiant</h3></div>
              ) : (
                  ...
                  {(classDetail.students??[]).map(s => (
                    <div key={s.id} className="card">
                      <div className="card-body" style={{ display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <button className="btn btn-danger btn-sm" style={{ padding:'2px 8px', fontSize:11 }} onClick={() => onRemoveStudent(s.id,s.name)}>Retirer</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {detailTab==='enrollments' && (
            <div>
              <div className="form-group" style={{ maxWidth:420, marginBottom:24 }}>
                <label className="form-label">Selectionner un cours</label>
                <select className="form-select" value={enrollCourseId||''} onChange={e=>onLoadEnrolled(parseInt(e.target.value))}>
                  <option value="">-- Choisir un cours --</option>
                  {(classDetail.courses??[]).map(c=><option key={c.id} value={c.id}>{c.title}</option>)}
                </select>
              </div>
              {enrollCourseId && (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }}>
                  <div>
                    ...
                    {enrolled.length===0 ? <div style={{ color:'var(--text-muted)', fontSize:13 }}>Aucun etudiant.</div>
                    : enrolled.map(s => (
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <button className="btn btn-primary btn-sm" style={{ marginLeft:'auto' }} onClick={() => onCreateUser(tab==='teachers'?'teacher':tab==='admins'?'admin':'student')}>
          + Nouvel utilisateur
        </button>
      </div>
...
function ResultsTab({ classId }) {
  ...
  useEffect(() => {
    api.get(`/classes/${classId}/results`).then(r=>setData(r.data)).catch(()=>{}).finally(()=>setLoad(false))
  }, [classId])
            ...
            <tr key={s.student_id} style={{ background:i%2===0?'white':'#f8fafc' }}>
              ...
              {e.map(x => { const r=s.exams[x.id]; return (
                <td key={`e${x.id}`} style={{ padding:'9px 8px', textAlign:'center' }}>
                  {!r?.submitted?<span style={{color:'#94a3b8'}}>NS</span>:!r.graded?<span style={{color:'#f59e0b'}}>En cours</span>:<span style={{fontWeight:700,color:r.score/r.max>=0.5?'#16a34a':'#d97706'}}>{r.score}/{r.max}</span>}
                </td>
              )})}
              {h.map(x => { const r=s.homeworks[x.id]; return (
                <td key={`h${x.id}`} style={{ padding:'9px 8px', textAlign:'center' }}>
                  {!r?.submitted?<span style={{color:'#94a3b8'}}>NS</span>:!r.graded?<span style={{color:'#f59e0b'}}>En cours</span>:<span style={{fontWeight:700,color:r.score/r.max>=0.5?'#16a34a':'#d97706'}}>{r.score}/{r.max}{r.late?' !':''}</span>}
                </td>
              )})}
            </tr>
          ))}
