 ...
 *
 * 1. ANNÉES ACADÉMIQUES — bouton "📅 Années académiques" dans le header
 *    → ouvre un modal dédié pour créer/modifier/supprimer des années librement
 *    → plus limité à un seul choix dans le select classe
 *
 * 2. THUMBNAIL — uploadable PENDANT la création du cours
 *    → ThumbnailField inline (sans import externe)
 *    → si l'image est choisie AVANT la création : stockée en pendingThumb,
 *      puis uploadée automatiquement juste après la création du cours
 *    → si choisie APRÈS (cours déjà créé) : uploadée immédiatement
 */
...
/* ════════════════════════════════════════════════
   ThumbnailField — upload image inline dans le modal cours
   Fonctionne en 2 modes :
   - courseId connu  → upload immédiat vers /admin/courses/:id/thumbnail
   - courseId null   → stocke le File dans pendingFile pour upload après création
════════════════════════════════════════════════ */
function ThumbnailField({ courseId, currentUrl, onFile, onUploaded }) {
  ...
  // Sync si currentUrl change (mode édition)
  ...
  const handle = async (file) => {
    ...
    if (courseId) {
      ...
      // Cours pas encore créé → donner le fichier au parent
      onFile?.(file)
    }
  }
...
export default function AdminDashboard() {
  ...
  const [yearModal,   setYearModal]   = useState(false)   // ← NOUVEAU
  ...
  const [editYear,    setEditYear]    = useState(null)  // ← édition d'une année dans le modal dédié
  ...
  const [pendingThumb,  setPendingThumb]  = useState(null)  // File en attente avant création
    ...
    try {
      ...
      if (editYear) {
        ...
        setYears(prev => prev.map(y =>
          y.id === editYear.id ? data :
          (newYearForm.is_current ? { ...y, is_current:false } : y)
        ))
        ...
        data = r.data
        setYears(prev => {
          const updated = newYearForm.is_current
  ...
  const deleteYear = async yr => {
    if (!confirm(`Supprimer l'année "${yr.name}" ?`)) return
    try {
  ...
  const saveUser = async e => {
    ...
    try {
      ...
      flash(editUser ? 'Utilisateur modifié' : 'Utilisateur créé')
      setUserModal(false); loadAll()
    } catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  const toggleActive = async u => {
    await api.put(`/admin/users/${u.id}`, { is_active:!u.is_active })
    flash(`Compte ${u.is_active?'désactivé':'activé'}`); loadAll()
  }
  const deleteUser = async id => {
    if (!confirm('Supprimer cet utilisateur ?')) return
    try { await api.delete(`/admin/users/${id}`); flash('Utilisateur supprimé'); loadAll() }
    catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  ...
  const saveCourse = async e => {
    ...
    try {
      if (editCourse) {
        ...
        // Si une image était déjà choisie avant → l'uploader maintenant
        if (pendingThumb instanceof File) {
          ...
          } catch { flash('Cours créé — image non uploadée','success') }
        } else {
          flash('Cours créé ! Vous pouvez ajouter une image puis fermer.','success')
        }
      }
    } catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  ...
  const enrollStudent = async studentId => {
    try { await api.post(`/admin/courses/${enrollCourseId}/enroll`, { student_ids:[studentId] }); flash('Étudiant inscrit'); loadEnrolled(enrollCourseId) }
    catch (err) { flash(err.response?.data?.detail || 'Erreur','error') }
  }
  const unenrollStudent = async studentId => {
    await api.delete(`/admin/courses/${enrollCourseId}/students/${studentId}`)
    flash('Étudiant retiré'); loadEnrolled(enrollCourseId)
  }
    ...
    <>
      ...
      {yearModal && (
        <div className="modal-overlay" onClick={() => setYearModal(false)}>
          <div className="modal" style={{maxWidth:620,width:'95vw',maxHeight:'90vh',overflowY:'auto'}}
            ...
            <div className="modal-body">
              ...
              <div style={{marginBottom:24}}>
                ...
                {years.length===0 ? (
                  <div style={{color:'var(--text-muted)',fontSize:13,padding:16,background:'#f8fafc',borderRadius:10}}>
                    Aucune année. Créez-en une ci-dessous.
                  </div>
                ) : years.map(yr => (
                  <div key={yr.id} style={{
              ...
              <div style={{padding:16,background:'#f0f9ff',borderRadius:12,border:'1px solid #bae6fd'}}>
                <div style={{fontWeight:700,fontSize:13,color:'var(--navy)',marginBottom:14}}>
                  {editYear ? `✏️ Modifier "${editYear.name}"` : '➕ Nouvelle année'}
                </div>
                <div className="form-group">
        ...
        <div className="modal-overlay" onClick={() => setClassModal(false)}>
          <div className="modal" style={{maxWidth:560,width:'95vw',maxHeight:'92vh',overflowY:'auto'}}
            ...
            <div className="modal-body">
              ...
              {/* Année académique + création inline */}
              <div className="form-group">
              ...
              <div className="modal-body">
                ...
                {savedCourseId && (
                  <div style={{padding:'10px 14px',borderRadius:10,background:'#f0fdf4',border:'1px solid #bbf7d0',color:'#16a34a',fontSize:13,fontWeight:600}}>
                    ✓ Cours créé ! Modifiez l'image si besoin puis fermez.
                  </div>
                )}
              </div>
              <div className="modal-footer">
...
function UsersView({ allUsers, onBack, onCreateUser, onEditUser, onToggleActive, onDeleteUser }) {
  const [tab,setTab]=[useState('students')].flat()
  const [search,setSearch]=useState('')
