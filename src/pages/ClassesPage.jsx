      ...
      <div style={{ padding: '18px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--navy)', marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {c.name}
            </div>
            {c.code && (
          ...
          <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 700, background: c.is_active ? '#dcfce7' : '#f1f5f9', color: c.is_active ? '#166534' : '#64748b', flexShrink: 0, marginLeft: 8 }}>
        ...
        <div style={{ marginBottom: isAdmin ? 12 : 0 }}>
          ...
          <div style={{ height: 5, background: '#f1f5f9', borderRadius: 4, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${pct}%`, borderRadius: 4, background: full ? '#ef4444' : color, transition: 'width .4s' }} />
          </div>
        </div>

        {isAdmin && (
          <div style={{ display: 'flex', gap: 6 }} onClick={e => e.stopPropagation()}>
            <button className="btn btn-outline btn-sm" style={{ fontSize: 11, padding: '3px 10px' }} onClick={onEdit}>✏️ Modifier</button>
            <button className="btn btn-danger btn-sm"  style={{ fontSize: 11, padding: '3px 10px' }} onClick={onDelete}>🗑️</button>
          </div>
        )}
      </div>
    </div>
...
export default function ClassesPage() {
  ...
  const flash = useCallback((text, type = 'success') => {
    ...
    setMsg({ text, type })
    flashTimerRef.current = setTimeout(() => setMsg({ text: '', type: '' }), 4000)
  }, [])

  // ── FIX #5 : load avec useCallback pour éviter boucle infinie ──
  ...
  const saveClass = useCallback(async e => {
    e.preventDefault()
  ...
  }, [form, editing, flash, load])

  const deleteClass = useCallback(async (id, e) => {
    e?.stopPropagation()
  ...
  // ── FIX #11 : useMemo pour filteredClasses ──
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <h3>
            {search ? 'Aucun résultat' : isStudent ? 'Aucune classe assignée' : isTeacher ? 'Aucune classe assignée' : 'Aucune classe'}
          </h3>
          <p>
            {search ? `Aucune classe pour "${search}".` : isAdmin ? 'Créez la première classe.' : 'Contactez votre administrateur.'}
          </p>
          {isAdmin && !search && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: 16 }}>
          {filtered.map(cls => (
            <ClassCard
              key={cls.id} cls={cls} isAdmin={isAdmin}
          ...
          <div className="modal" style={{ maxWidth: 560, width: '95vw', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <form onSubmit={saveClass}>
              <div className="modal-body">
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="L1-INFO" />
                  </div>
                </div>
                <div className="form-row">
                  <div className="form-group">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="number" min={1} value={form.max_students} onChange={e => setForm(f => ({ ...f, max_students: e.target.value }))} />
                  </div>
                </div>
                <div className="form-row">
                  <div className="form-group">
                    <label className="form-label">Année académique</label>
                    <select className="form-select" value={form.academic_year_id} onChange={e => setForm(f => ({ ...f, academic_year_id: e.target.value }))}>
                ...
                <div className="form-group">
                  ...
                  <textarea className="form-input" rows={2} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} style={{ resize: 'vertical' }} placeholder="Description optionnelle…" />
                </div>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary">{editing ? 'Enregistrer' : 'Créer la classe'}</button>
              </div>
            </form>
          </div>
        </div>
