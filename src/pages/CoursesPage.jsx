...
/* ── Palettes & icônes ── */
const CAT_GRADIENTS = [
  ['#1e3a5f', '#0ea5e9'], ['#1a2e1a', '#22c55e'],
  ['#2e1a1a', '#ef4444'], ['#2e2a1a', '#f59e0b'],
  ['#1a1a2e', '#8b5cf6'], ['#1a2e2e', '#14b8a6'],
  ['#2e1a2e', '#ec4899'],
]
const catGradient = (_n, id = 0) => CAT_GRADIENTS[id % CAT_GRADIENTS.length]
  ...
  return (
    <div style={{ fontFamily:"'Sora','DM Sans',sans-serif" }}>
      ...
      <style>{`
        ...
        .course-card-xl:hover .card-img-zoom { transform:scale(1.08); }
        .course-card-xl:hover .card-cta-arrow { transform:translateX(5px); opacity:1; }
        .card-cta-arrow { transition:transform .3s,opacity .3s; opacity:.45; }

        .card-img-xl { height:220px; position:relative; overflow:hidden; flex-shrink:0; }
        .card-img-zoom {
        ...
        .enroll-btn {
          font-size:13px; font-weight:700; padding:9px 20px; border-radius:12px;
        ...
        .pdf-btn {
          ...
          background:white; cursor:pointer; color:#ef4444; transition:all .15s;
        }
        .pdf-btn:hover { background:#fef2f2; border-color:#fca5a5; transform:scale(1.03); }
        .course-list-row {
          background:white; border-radius:18px; border:1px solid #e8ecf4;
          padding:16px 20px; cursor:pointer; display:flex; align-items:center; gap:18px;
        ...
        .course-list-row:hover {
          ...
          transform:translateX(5px); border-color:rgba(59,130,246,.3);
        }
      `}</style>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          {filtered.map((c,idx) => (
            <CourseCardXL key={c.id} course={c} idx={idx}
              isStudent={isStudent} isTeacher={isTeacher} isAdmin={isAdmin}
              myOnly={myOnly} enrolling={enrolling}
              onEnroll={enroll} onOpenPdf={openPdf}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {filtered.map((c,idx) => (
            <CourseListRow key={c.id} course={c} idx={idx}
              isStudent={isStudent} isTeacher={isTeacher} isAdmin={isAdmin}
              myOnly={myOnly} enrolling={enrolling}
              onEnroll={enroll} onOpenPdf={openPdf}
              onClick={() => navigate(`/courses/${c.id}`)}
  ...
  return (
    <div className="course-card-xl" onClick={onClick}>
      ...
      <div className="card-img-xl">
        ...
        {/* Grande icône si pas de thumbnail */}
        {!thumb && (
          <div style={{
            position:'absolute',inset:0,
        ...
        {/* Titre sur l'image si thumbnail */}
        {thumb && (
      ...
      <div style={{padding:'18px 20px 20px',flex:1,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{
          ...
          paddingBottom:14,borderBottom:'1px solid #f1f5f9',flexWrap:'wrap',
        }}>
    ...
    <div className="course-list-row" onClick={onClick}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:12,color:'#64748b',marginTop:4,fontWeight:500}}>
          {c.teacher_name}{c.lesson_count>0&&<> · {c.lesson_count} leçon(s)</>}{c.category?.name&&<> · {c.category.name}</>}
        </div>
        {isStudent && c.enrolled && pct>0 && (
